...
contract Methods is NodeSales {
    ...
    function mint(address currency, string memory referralCode, uint16 quantity) nonReentrant external {
    ...
    function setPrice(uint256 _price, address _token) external onlyOwner {
        price = _price;
        token = _token;
    }
    ...
    function setAcceptedCurrency(address currency) external onlyOwner {
        acceptedCurrencies[currency] = !acceptedCurrencies[currency];
    }
}
